# squad-animation-private

## Objectifs de la Squad
Faire des introductions sur des sujets technique sur 30 à 45 min max.

## evenements potentiels

### Par Angélique

- git: les commandes de base, clone, push pull, commit
- github: utilisation, github cli, projects, issues , workflow
- docker, docker-compose
- tests unitaires
- tous les types de tests et quand utiliser quoi
- spring boot
- elastic/kibana
- SPA / API REST Single Page Application, architecture habituelle (pas prêt tout de suite)

### Par Pierre

- 1er sujet : Prise de parole  en public / Comment passer d'un propos ennuyant à une parole captivante et engageante pour l'audience ? 
-- Sujets abordés : Système 1 et 2 de Daniel Khanement, Outil de la structure d'une histoire / Story telling, Figure de style

- 2ème sujet : Prise de parole  en public / Comment transmettre ses idées et projets à un public non expert ? 
-- Sujets abordés : Ethos / Pathos/ Logos, Méthode pour connaître son public pour s'adapter. 

- 3ème sujet : Prise de parole  en public / Comment casser son syndrome de l'imposteur et affirmer son autorité par la prise de parole ? 
-- Sujets abordés : Confiance en soi / Estime de soi / Posture et voix / Piliers de sécurité de la prise de parole. 

- 4ème sujet :Prise de parole  en public / Comment réussir à convaincre en moins de 10 minutes ? 
-- Sujets abordés : FUOM, les types d'arguments, la structure d'un argument. 

- 5ème sujet : Prise de parole  en public / Comment passer d'un rôle d'expert à un rôle de leader par la persuasion ? 
-- Sujets abordés : Pathos / Histoire / Emotions / Prosodie / Figures de style / Bénéfices attendus / biais cognitifs
